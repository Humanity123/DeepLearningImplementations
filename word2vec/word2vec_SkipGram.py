...
def generateBatch(batchSize, repeatWindow, windowSize):
	global dataIndex
	...
	for _ in range(span):
		...
		dataIndex = (dataIndex + 1)%len(data)
	for i in range(batchSize//repeatWindow):
		target = windowSize
		target_avoid = [target]
		for j in range(repeatWindow):
			while target in target_avoid:
				target = random.randint(0,span-1)
			target_avoid.append(target)
			batch[i*repeatWindow+j]    = buffer[windowSize]
			label[i*repeatWindow+j][0] = buffer[target]
		if len(buffer) != 0 and dataIndex == 0:
			startFresh(span, buffer)
		else :
			buffer.append(data[dataIndex])
...
with graph.as_default():
	...
	train_input = tf.placeholder(tf.int32, shape = [batchSize])
	...
	nce_loss    = tf.reduce_mean(tf.nn.nce_loss(weights, bias, train_label, embed, negSamples, volcabSize)) 
...
with tf.Session(graph = graph) as sess:
	...
	for epoch in range(1,epochs+1):
		batch, label = generateBatch(batchSize, repeatWindow, windowSize)
